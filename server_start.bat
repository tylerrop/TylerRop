...
netstat -anp tcp | FIND /I "0.0.0.0:8086" >NUL
IF ERRORLEVEL 1 goto NOTFOUND
echo.
echo  Another server is running on port 8086 cannot run Apache server
echo.
goto END

:NOTFOUND
echo  Port 8086 is free - OK to run server
...
set apacheit=%Disk%:%apachepath%bin\Apache_6.exe -f %apachepath%conf\httpd.conf -d %apachepath%.
...
echo.
echo  To view site type http://localhost:8086 into a browser
goto :END
